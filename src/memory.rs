...
///
/// This function is unsafe because the caller must guarantee that the complete physical
/// memory is mapped to virtual memory at the passed `physical_memory_offset`. Also,
/// this function must be only called once to avoid aliasing `&mut` references (which is
/// undefined behavior).
...
/// Translates the given virtual address to the mapped physical address, or `None` if the
/// address is not mapped.
///
/// This function is unsafe because the caller must guarantee that the complete physical
/// memory is mapped to virtual memory at the passed `physical_memory_offset`.
...
///
/// This function is unsafe because the caller must guarantee that the complete physical
/// memory is mapped to virtual memory at the passed `physical_memory_offset`. Also, this
/// function must be only called once to avoid aliasing `&mut` references (which is
/// undefined behaviour).
