...
const PANIC_LINE: u32 = 32; // Adjust this when moving the `panic!` call!

/// Compares a `fmt::Arguments` instance with the `MESSAGE` string.
///
/// To use this type, write the `fmt::Arguments` instance to it using the `write!` macro.
/// If a message component matches `MESSAGE`, the equals field is set to true.
...
pub extern "C" fn _start() -> ! {
    ...
    panic!(MESSAGE); // Must be in the line `PANIC_LINE`.
}
